...
e1000_recv(void)
{
  ...
  struct mbuf *tmp_buf;
  acquire(&e1000_lock);
  rx_desc = &(rx_ring[(regs[E1000_RDT] + 1) % TX_RING_SIZE]);
  rx_buf = rx_mbufs[(regs[E1000_RDT] + 1) % TX_RING_SIZE];
  while (rx_desc->status & E1000_RXD_STAT_DD)
  {
    if (rx_buf->len > rx_desc->length)
    {
      mbuftrim(rx_buf, rx_buf->len - rx_desc->length);
    }
    else if (rx_buf->len < rx_desc->length)
    {
      mbufput(rx_buf, rx_desc->length - rx_buf->len);
    }
    tmp_buf = rx_mbufs[(regs[E1000_RDT] + 1) % TX_RING_SIZE] = mbufalloc(0);
    rx_desc->addr = (uint64)tmp_buf->head;
    rx_desc->status = 0;
