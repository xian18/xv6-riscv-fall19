  ...
  *(np->tf) = *(p->tf);
  ...
  for (i = 0; i < NPVMAS; i++)
  {
    if (p->vmrs[i])
    {
      ...
      np->vmrs[i] = vmrdup(vmr);
      for (uint64 off = 0; off < vmr->length; off += PGSIZE)
      {
        pte_t *pte = walk(p->pagetable, vmr->uvaddr);
        if (pte && *pte & PTE_V)
        {
          uint64 pa = PTE2PA(*pte);
          int flags = PTE_FLAGS(*pte) & ~PTE_W;
          uvmunmap(p->pagetable, vmr->uvaddr + off, PGSIZE, 0);
          if (mappages(p->pagetable, vmr->uvaddr + off, PGSIZE, pa, flags) < 0)
          {
            freeproc(np);
            release(&np->lock);
            return -1;
          }
          if (mappages(np->pagetable, vmr->uvaddr + off, PGSIZE, pa, flags) < 0)
          {
            freeproc(np);
            release(&np->lock);
            return -1;
...
void exit(int status)
{
  ...
  for (int vmrd = 0; vmrd < NPVMAS; vmrd++)
  {
    if (p->vmrs[vmrd])
    {
      struct vmr_t *vmr = p->vmrs[vmrd];
      writebackvmr(vmr);
      uvmunmap(p->pagetable, vmr->uvaddr, vmr->length, 1);
      freevmr(vmr);
      p->vmrs[vmrd] = 0;
    }
  }
