...
bd_malloc(uint64 nbytes)
{
  ...
  for (; k > fk; k--)
  {
    ...
    //char *q = addr(k - 1, blk_index(k - 1, p) ^ 1);
...
void bd_free(void *p)
{
  ...
  for (k = size(p); k < MAXSIZE; k++)
  {
    ...
    bit_xor(bd_sizes[k].alloc, bi >> 1); // free p at size k
...
int bd_initfree_pair(int k, int bi, int dir)
{
  int buddy = (bi % 2 == 0) ? bi + 1 : bi - 1;
  int free = 0;
  if (bit_isset(bd_sizes[k].alloc, bi >> 1))
...
void bd_init(void *base, void *end)
{
  ...
  for (int k = 0; k < nsizes; k++)
  {
    ...
    sz = ROUNDUP(sizeof(char) * ROUNDUP(NBLK(k), 8) / 8, 2) / 2;
    bd_sizes[k].alloc = p;
